...
class case_advance_payment_inv(osv.osv_memory):
    _name = "case.advance.payment.inv"
    _description = "Case Advance Payment Invoice"

    _columns = {
        'advance_payment_method':fields.selection(
            [('all', 'Invoice the whole Case'), ('fixed','Fixed price (deposit)'),
                ('task', 'Some Tasks')],
            'What do you want to invoice?', required=True,
            help="""Use All to create the final invoice.
                Use Fixed Price to invoice a specific amound in advance.
                Use Some Tasks to invoice a selection of the Tasks."""),
    ...
    def _create_invoices(self, cr, uid, inv_values, case_id, context=None):
        inv_obj = self.pool.get('account.invoice')
        case_obj = self.pool.get('case.sheet')
    ...
    def create_invoices(self, cr, uid, ids, context=None):
        ...
        if wizard.advance_payment_method == 'all':
            ...
            res = case_obj.manual_invoice(cr, uid, case_ids, context)
